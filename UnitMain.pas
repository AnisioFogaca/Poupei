unit UnitMain;

interface

uses
  System.SysUtils, System.Types, System.UITypes, System.Classes, System.Variants,
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Graphics, FMX.Dialogs, FMX.StdCtrls,
  FMX.Objects, FMX.Controls.Presentation, FMX.Layouts, FMX.TabControl, FMX.Edit;

type
  TFrmLogin = class(TForm)
    TabControl: TTabControl;
    TabInitial: TTabItem;
    TabLogin: TTabItem;
    TabNewProfile: TTabItem;
    Layout1: TLayout;
    Image1: TImage;
    Label1: TLabel;
    rectLogin: TRectangle;
    btnAcessoLogin: TSpeedButton;
    backgroundInitial: TRectangle;
    Text: TLayout;
    buttons: TLayout;
    Rectangle1: TRectangle;
    SpeedButton2: TSpeedButton;
    backgroundLogin: TRectangle;
    Layout2: TLayout;
    Image2: TImage;
    Layout3: TLayout;
    Rectangle3: TRectangle;
    SpeedButton3: TSpeedButton;
    Layout4: TLayout;
    Label2: TLabel;
    Edit1: TEdit;
    Edit2: TEdit;
    Label3: TLabel;
    Rectangle2: TRectangle;
    Layout5: TLayout;
    Image3: TImage;
    Layout6: TLayout;
    Rectangle4: TRectangle;
    SpeedButton4: TSpeedButton;
    Edit3: TEdit;
    Edit4: TEdit;
    Layout7: TLayout;
    Label4: TLabel;
    Label5: TLabel;
    Edit5: TEdit;
    Edit6: TEdit;
    procedure btnAcessoLoginClick(Sender: TObject);
    procedure SpeedButton2Click(Sender: TObject);
    procedure Label3Click(Sender: TObject);
    procedure Label5Click(Sender: TObject);
    procedure FormShow(Sender: TObject);
  private
    { Private declarations }
  public
    { Public declarations }
  end;

var
  FrmLogin: TFrmLogin;

implementation

{$R *.fmx}

procedure TFrmLogin.btnAcessoLoginClick(Sender: TObject);
begin
  tabcontrol.GotoVisibleTab(1);
end;

procedure TFrmLogin.FormShow(Sender: TObject);
begin
  tabcontrol.ActiveTab := tabInitial;
end;

procedure TFrmLogin.Label3Click(Sender: TObject);
begin
  tabcontrol.GotoVisibleTab(2);
end;

procedure TFrmLogin.Label5Click(Sender: TObject);
begin
  tabcontrol.GotoVisibleTab(1);
end;

procedure TFrmLogin.SpeedButton2Click(Sender: TObject);
begin
  tabcontrol.GotoVisibleTab(2);
end;

end.
